...
class LeNet:
    ...
    def build(width, height, depth, classes):
        ...
        model.add(Conv2D(2, (11, 11), padding="same",
...
def train():
    ...
    imagePaths = []
    paths = 'C:/Users/zyh/Desktop/train'
    ...
    for imagePath in imagePaths:
        ...
        cv2.dilate(image, image, element)
        image = img_to_array(image)
        data.append(image)
        label = imagePath.split(os.path.sep)[-2]
        #masks = 1 if masks == "in" else 0
        labels.append(label)
    ...
    model = LeNet.build(width=128, height=128, depth=1, classes=10)
    ...
    model.save('C:/Users/zyh/Desktop/simple_nn_glass.h5', save_format="h5")
...
def batch_test():
    imagePaths = []
    paths = 'C:/Users/zyh/Desktop/test'
    # grab the images paths and randomly shuffle them
    imagePaths = sorted(list(getFileList(paths, imagePaths)))

    # load the trained convolutional neural network
    print("[INFO] loading network...")
    model = load_model('C:/Users/zyh/Desktop/simple_nn_glass.h5')
    ...
    for imagePath in imagePaths:
        ...
        o_label = imagePath.split(os.path.sep)[-2]
        res = model.predict(image)
        p_label = np.argmax(res[0])

        print(index)

        if p_label == 9 and int(o_label) != 9:
            print(imagePath)
            error_f += 1

        if int(o_label) == 9 and p_label != 9:
            print(imagePath)
            true_f += 1

        index += 1
    label = "Red Panda Not Red Panda"



def test():
    # load the images
    image = cv2.imread('C:/Users/zyh/Desktop/124.jpg', cv2.IMREAD_GRAYSCALE)
    orig = image.copy()
...
def h5_to_pb():
    model = load_model('D:\\2023020302.h5', custom_objects={'KerasLayer': hub.KerasLayer, 'Dense': Dense},  compile=False)
    model.summary()
    ...
    tf.io.write_graph(graph_or_graph_def=frozen_func.graph, logdir="D:\\", name="2023020302.pb", as_text=False)
