...
myGene = trainGenerator(batch_size,'data/train_another','image_jpg','label_jpg', data_gen_args, save_to_dir = None, image_color_mode='rgb', mask_color_mode='grayscale') # 'rgb', 'rgb', 

model_checkpoint = ModelCheckpoint('fcn32_another.hdf5', monitor='loss', verbose=1, save_best_only=True)

model = FCN8(1, 256, 256)
history = model.fit_generator(myGene, 	# 训练数据
                    steps_per_epoch = 30, #len(train_path) # batch_size,
                    epochs = epochs,
                    #verbose = 1,
...
tf.saved_model.save(model, "save_test_another")
